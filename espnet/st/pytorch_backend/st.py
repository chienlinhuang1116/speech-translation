...
def train(args):
    ...
    load_tr = LoadInputsAndTargets(mode='asr', load_output=True, 
                                    preprocess_conf=args.preprocess_conf,
                                    preprocess_args={'train': True},
                                    langs_dict=args.langs_dict,
                                    src_lang=src_lang)
    load_cv = LoadInputsAndTargets(mode='asr', load_output=True, 
                                    preprocess_conf=args.preprocess_conf,
                ...
                if args.recog_and_trans:    # for cross
                    ...
                    if args.beam_search_type == 'sum':
                        ...
                        new_js[name]['output'].append(
                            add_results_to_json(js[name], nbest_hyps_asr, 
                            train_args.char_list, output_idx=1)['output'][0])
                    else:
