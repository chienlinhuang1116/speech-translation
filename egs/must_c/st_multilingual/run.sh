...
echo "| suffix for dictionary = ${suffix}"
echo "| nbpe = ${nbpe}"
echo "| ngpu = ${ngpu}"
...
if [[ ${stage} -le 1 ]] && [[ ${stop_stage} -ge 1 ]]; then
    ...
    for lang in $(echo ${tgt_langs} | tr '_' ' '); do
        ...
        echo "Finished processing features for ${lang}"
    done
fi
...
if [[ ${stage} -le 2 ]] && [[ ${stop_stage} -ge 2 ]]; then
    ...
    offset=$(wc -l < ${dict})
    echo "| offset= ${offset}"
    input_path=data/lang_1spm/input_en-${tgt_langs}.${tgt_langs}.${tgt_case}${suffix}.txt
